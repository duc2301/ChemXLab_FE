...
const ATOM_PROPS: any = {
  ...
  O: { color: 0xff0d0d, radius: 0.6, label: "O" },
...
function createBond(aObj: any, bObj: any, opts: any = {}) {
  ...
  const material =
    opts.material !== undefined ? opts.material : bondMat.clone();
  ...
  const getRadius = (o: any) =>
    o.userData && o.userData.radius ? o.userData.radius : 0;
...
const Reaction = React.forwardRef((props, ref) => {
  ...
  useImperativeHandle(
    ...
    () => ({
      startReaction: () => {
        if (startReactionRef.current) startReactionRef.current();
      },
      openMolecular: () => {
        if (openMolecularRef.current) openMolecularRef.current();
      },
      reset: () => {
  ...
  useEffect(() => {
    ...
    function createLabTable(tableTopY: number) {
      ...
      const tableTop = new THREE.Mesh(
        new THREE.BoxGeometry(100, 3.33, 50),
        tableTopMat
      );
      ...
      const legPositions = [
        new THREE.Vector3(44.66, tableTop.position.y - 25, 22.33),
        new THREE.Vector3(44.66, tableTop.position.y - 25, -22.33),
        new THREE.Vector3(-44.66, tableTop.position.y - 25, 22.33),
        new THREE.Vector3(-44.66, tableTop.position.y - 25, -22.33),
      ];
    ...
    const base = new THREE.Mesh(
      new THREE.BoxGeometry(9.33, 1.33, 13.33),
      metalMat
    );
    base.position.set(-8.7, TABLE_TOP_Y_LEVEL + 0.665, 4.365);
    standGroup.add(base);
    const rod = new THREE.Mesh(
      new THREE.CylinderGeometry(0.4, 0.4, 19, 12),
      metalMat
    );
    rod.position.set(-8.7, TABLE_TOP_Y_LEVEL + 9.5, 0);
    standGroup.add(rod);
    const bosshead = new THREE.Mesh(
      new THREE.BoxGeometry(2, 2.67, 2),
      metalMat
    );
    bosshead.position.set(-8.7, CLAMP_Y_LEVEL, 1.5);
    standGroup.add(bosshead);
    const clampGroup = new THREE.Group();
    const clampRod = new THREE.Mesh(
      new THREE.CylinderGeometry(0.267, 0.267, 7, 12),
      metalMat
    );
    clampRod.rotation.z = Math.PI / 2;
    clampRod.position.x = 3.5;
    clampGroup.add(clampRod);
    const prong1 = new THREE.Mesh(
      new THREE.BoxGeometry(2, 0.33, 0.33),
      rubberMat
    );
    prong1.position.set(7, 0, -0.5);
    ...
    prong2.position.set(7, 0, 0.5);
    prong2.rotation.y = -Math.PI / 6;
    clampGroup.add(prong2);
    clampGroup.position.set(-7.7, CLAMP_Y_LEVEL, 1.5);
    ...
    const waterBottomGeomSimple = new THREE.SphereGeometry(
      waterRadius,
      32,
      16,
      0,
    ...
    if (waterHeightInTube > 0) {
      ...
      const waterBodyGeom = new THREE.CylinderGeometry(
        waterRadius,
        waterRadius,
        waterBodyHeight,
        32,
    ...
    [
      ...
      steamEffect,
      bubbleEffect,
      boilingBubblesEffect,
      hydrogenFlameEffect,
      sparkEffect,
    ].forEach((p) => {
      if (p) p.frustumCulled = false;
    });
    ...
    function animate() {
      ...
      if (sphereState === "falling") {
        ...
        if (sodiumSphere.position.y <= waterLevelY + sodiumSize * 0.3) {
          ...
          fireEffect.start();
          if (fireEffect.emitter) fireEffect.emitter.rate = 150.0;
          steamEffect.start();
          if (steamEffect.emitter) steamEffect.emitter.rate = 80.0;
          bubbleEffect.start();
          boilingBubblesEffect.start();
          hydrogenFlameEffect.start();
          sparkEffect.start();
          if (sparkEffect.emitter) sparkEffect.emitter.rate = 60.0;
          reactionTimer = 0.0;
        ...
        if (fireEffect && fireEffect.emitter)
          fireEffect.emitter.rate = 150.0 * fireIntensity;
        if (steamEffect && steamEffect.emitter)
          steamEffect.emitter.rate = 80.0 * fireIntensity;
        if (sparkEffect && sparkEffect.emitter)
          sparkEffect.emitter.rate = 60.0 * fireIntensity;
        ...
        fireLight.intensity =
          (Math.random() * 0.3 + 0.2) * (1.0 - progress * 0.7);
        ...
        const speed = Math.sqrt(
          sodiumVelocityX * sodiumVelocityX + sodiumVelocityZ * sodiumVelocityZ
        );
        if (speed > maxSpeed) {
        ...
        const distFromCenter = Math.sqrt(
          sodiumSphere.position.x * sodiumSphere.position.x +
            sodiumSphere.position.z * sodiumSphere.position.z
        );
        if (distFromCenter > maxRadius) {
          const angle = Math.atan2(
            sodiumSphere.position.z,
            sodiumSphere.position.x
          );
          ...
          const dotProduct =
            sodiumVelocityX * normalX + sodiumVelocityZ * normalZ;
        ...
        sodiumSphere.rotation.x =
          Math.sin(elapsedTime * 25) * vibrationAmount * (1.0 + speedFactor);
        sodiumSphere.rotation.z =
          Math.cos(elapsedTime * 28) * vibrationAmount * (1.0 + speedFactor);
        sodiumSphere.rotation.y += delta * speed * 2.0;

        (waterMaterial.uniforms.u_wave_origin as any).value.x =
          sodiumSphere.position.x;
        (waterMaterial.uniforms.u_wave_origin as any).value.y =
          sodiumSphere.position.z;
        const effectiveVelocity = Math.sqrt(
          sodiumVelocityX * sodiumVelocityX + sodiumVelocityZ * sodiumVelocityZ
        );
        const velocityWithImpact = effectiveVelocity + waterImpactForce * 0.3;
        (waterMaterial.uniforms.u_sphere_velocity as any).value.x =
          sodiumVelocityX *
          (velocityWithImpact / Math.max(effectiveVelocity, 0.1));
        (waterMaterial.uniforms.u_sphere_velocity as any).value.y =
          sodiumVelocityZ *
          (velocityWithImpact / Math.max(effectiveVelocity, 0.1));
      ...
      try {
        ...
        steamEffect.update(delta);
        bubbleEffect.update(delta);
        boilingBubblesEffect.update(delta);
        hydrogenFlameEffect.update(delta);
        sparkEffect.update(delta);
      } catch (e) {}
    ...
    function onTestTubeClick(event: MouseEvent) {
      ...
      const isClickingMolecularModal =
        molecularModal &&
        (molecularModal.contains(event.target as Node) ||
          event.target === molecularModal);
      if (isHovering) {
    ...
    if (btnStartReaction)
      btnStartReaction.addEventListener("click", startReactionFn);
    ...
    function initMolecularScene() {
      ...
      renderer_mol = new THREE.WebGLRenderer({
        canvas: canvas,
        antialias: true,
        alpha: true,
      });
    ...
    function animateMolecularScene() {
      ...
      if (renderer_mol && camera_mol && scene_mol)
        renderer_mol.render(scene_mol, camera_mol);
      if (labelRenderer && camera_mol && scene_mol)
        labelRenderer.render(scene_mol, camera_mol);
    }
    ...
    function startReactionAnimation() {
        ...
        .onStart(() => {
          ...
          bonds.forEach((b: any, i) => {
            ...
            new TWEEN.Tween(shakeAnim)
              ...
              .repeat(3)
              .onUpdate(() => {
                b.rotation.z = Math.sin(shakeAnim.x * Math.PI * 4) * 0.2;
              })
              .start();
          ...
          setTimeout(() => {
            ...
            Object.keys(atoms).forEach((k) =>
              detachAtomToScene(atoms[k], scene_mol!)
            );
          }, 300);
        });
      ...
      tBreak.onComplete(() => {
        new TWEEN.Tween(atoms.h1a.parent.position)
          .to(h2_pos.h1a, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .start();
        new TWEEN.Tween(atoms.h2a.parent.position)
          .to(h2_pos.h2a, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .chain(tFormH2)
          .start();
        new TWEEN.Tween(atoms.k1.parent.position)
          .to(koh1_pos.k1, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .start();
        new TWEEN.Tween(atoms.o1.parent.position)
          .to(koh1_pos.o1, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .start();
        new TWEEN.Tween(atoms.h1b.parent.position)
          .to(koh1_pos.h1b, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .chain(tFormKOH1)
          .start();
        new TWEEN.Tween(atoms.k2.parent.position)
          .to(koh2_pos.k2, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .start();
        new TWEEN.Tween(atoms.o2.parent.position)
          .to(koh2_pos.o2, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .start();
        new TWEEN.Tween(atoms.h2b.parent.position)
          .to(koh2_pos.h2b, duration)
          .easing(TWEEN.Easing.Back.InOut)
          .chain(tFormKOH2)
          .start();
      });
